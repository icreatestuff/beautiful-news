...
/**
 * Freeform - Freeform Mailinglist Fieldtype
 * ExpressionEngine fieldtype interface licensed for use by EllisLab, Inc.
 *
 * @package 	Solspace:Freeform
 * @author		Solspace DevTeam
 * @filesource 	./system/expressionengine/third_party/freeform/freeform_ft.mailinglist.php
 */
...
class Mailinglist_freeform_ft extends Freeform_base_ft
{
	...
	public function mailinglist_module_enabled()
	{
		...
		if ( isset($this->EE->TMPL) AND isset($this->EE->TMPL->module_data['Mailinglist']) )
		{
	...
	public function get_lists ($show_all = FALSE)
	{
		...
		$this->EE->db->select(array('list_id','list_title'));
		...
		{
			$this->EE->db->where_in( 'list_id', $this->settings['show_lists_by_default'] );
		}
	...
	public function save ($data)
	{
		...
		if ( $this->EE->input->get_post('email') )
		{
	...
	private function subscribe_to_lists ($data = array(), $emails = array() )
	{
		...
		foreach ( $emails as $email )
		{
			...
			if ( ! empty( $uns ) )
			{
				$sql	= "DELETE FROM exp_mailing_list WHERE email = '" . $this->EE->db->escape_str($email) . "' AND list_id IN (" . implode( ',', $uns ) . ")";

				$this->EE->db->query( $sql );

				$sql	= "DELETE FROM exp_mailing_list_queue WHERE email = '" . $this->EE->db->escape_str($email) . "' AND list_id IN (" . implode( ',', $uns ) . ")";
