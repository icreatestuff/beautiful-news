 ...
 * @package 	Solspace:Freeform
 * @author		Solspace DevTeam
 * @filesource 	./system/expressionengine/third_party/freeform/freeform_base_ft.php
 */
...
class Freeform_base_ft
{
	...
	public function __construct()
	{
		...
		if ( ! isset($this->EE->session->cache['freeform_ft_cache'][$site_id]))
		{
			$this->EE->session->cache['freeform_ft_cache'][$site_id] = array();
		}

		//private to this parent constructor
		$this->cache =& $this->EE->session->cache['freeform_ft_cache'][$site_id];
	}
	//END __construct
										...
										$desc = 'multi_list_items_desc' )
	{
		...
		$this->EE->table->add_row(
			...
			array(
				'data'	=> $this->EE->load->view('multi_item_row.html', $vars, TRUE),
	...
	protected function save_multi_item_row_settings ($data, $prefix = FALSE, $validate = FALSE)
	{
		...
		$list_type = $this->EE->input->post($prefix . 'list_type', TRUE);
		...
		{
			$keys 	= $this->EE->input->post(
				$prefix . 'list_value_holder_input',
				TRUE
			);

			$values = $this->EE->input->post(
		...
		{
			$temp_settings = $this->EE->input->post(
				$prefix . 'list_holder_input',
				TRUE
			);

			//simple wimple, no blanky wanky
	...
	protected function get_field_options ($prefix = FALSE, $output_mode = TRUE)
	{
		...
		else if ($this->settings[$prefix . 'list_type'] == 'channel_field')
		{
			...
			if ($field_id == 'u')
			{
				$this->EE->db->select('url_title as output_item');
				$this->EE->db->from('channel_titles');
			}
			else if ($field_id == 't')
			{
				$this->EE->db->select('title as output_item');
				$this->EE->db->from('channel_titles');
			}
			else
			{
				$this->EE->db->from('channel_data');
				$this->EE->db->select('field_id_'.$field_id . ' as output_item');
			}

			$this->EE->db->select('entry_id');
			$this->EE->db->where('channel_id', $channel_id);
			$pop_query = $this->EE->db->get();
	...
	protected function form_table_name ($form_id)
	{
		...
		return $this->EE->freeform_form_model->table_name($form_id);
	}
	//END form_table_name
	...
	{
		return $this->EE->functions->encode_ee_tags(form_prep($str), TRUE);
	}
