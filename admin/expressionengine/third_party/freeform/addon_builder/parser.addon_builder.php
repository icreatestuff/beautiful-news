...
if (APP_VER < 2.0)
{
	eval('class Addon_builder_parser_base_freeform extends Template { }');
}
else
{
	eval('class Addon_builder_parser_base_freeform extends EE_Template { }');
}

class Addon_builder_parser_freeform extends Addon_builder_parser_base_freeform
{
	private $old_get = '';
    ...
    public function __construct()
    {
    	...
    	// --------------------------------------------
        //  Solves the problem of redirect links (?URL=) being added by Typography in a CP request
        // --------------------------------------------
    	...
    	// --------------------------------------------
        //  ExpressionEngine only loads snippets on PAGE and ACTION requests
        // --------------------------------------------
        
		if (APP_VER >= 2.0 && REQ == 'CP')
		{
			...
			if ($fresh->num_rows() > 0)
			{
				...
				foreach ($snippets as $name => $content)
				{
					$snippets[$name] = str_replace($var_keys, 
								array_values(ee()->config->_global_vars), $content);
				}
	...
	{   
		...
		if (APP_VER < 2.0)
		{
			$this->global_vars['last_segment'] = end($seg_array);
		}
		else
		{
		...
		if (APP_VER >= 2.0 && count(ee()->config->_global_vars) > 0)
		{
		...
		/** -------------------------------------
        /**  Parse Global Vars - EE 1.x and Solspace Modules (which use this for setting own globals)
        /** -------------------------------------*/
